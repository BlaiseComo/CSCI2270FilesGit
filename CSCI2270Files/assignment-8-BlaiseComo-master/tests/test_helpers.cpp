...
string test_shortestpath(string* name, int len, string* edge, int elen, string src, string dest)
{
    Graph g;
    testing::internal::CaptureStdout();
    string delimiter = "->";
    ...
    g.displayEdges();
    int com = g.findShortestPathBetweenBuildings(src, dest);
    //cout << "Number of Connected Components is: " << com << endl;
    string output = testing::internal::GetCapturedStdout();
    return output;
}
